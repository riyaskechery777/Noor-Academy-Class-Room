...
export default function StudentMessages() {
  ...
  const loadData = async () => {
    const [msgsRes, teachersRes] = await Promise.all([
      supabase.from('messages').select('*').or(`sender_id.eq.${user!.id},receiver_id.eq.${user!.id}`).order('created_at'),
      supabase.from('profiles').select('id, full_name, email').eq('role', 'teacher').eq('approved', true).limit(30),
    ]);
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    setSending(true);
    await supabase.from('messages').insert({ sender_id: user!.id, receiver_id: selected.id, content: newMsg.trim() });
    setSending(false);
    setNewMsg('');
    loadData();
  };
    ...
    <DashboardLayout role="student">
      <div className="h-[calc(100vh-8rem)] flex flex-col">
        <div className="mb-4">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Chat with your teachers.</p>
        </div>
        <div className="flex-1 flex gap-4 min-h-0">
          <div className="w-64 flex-shrink-0 card p-3 flex flex-col">
            <div className="relative mb-3">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
              <input type="text" placeholder="Search teachers..." value={search} onChange={e => setSearch(e.target.value)} className="input-field pl-9 py-2 text-sm" />
            </div>
            <div className="flex-1 overflow-y-auto space-y-1">
              {filteredTeachers.map(t => (
                <button key={t.id} onClick={() => setSelected(t)}
                  className={`w-full flex items-center gap-2.5 p-2.5 rounded-xl text-left transition-colors ${selected?.id === t.id ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                  <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                    {t.full_name?.charAt(0)?.toUpperCase()}
                  </div>
                  <p className={`text-sm font-medium truncate ${selected?.id === t.id ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-900 dark:text-white'}`}>{t.full_name}</p>
                </button>
              ))}
            </div>
          </div>
            ...
            {!selected ? (
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">Select a teacher to start chatting</p>
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {conversation.length === 0 ? (
                    <p className="text-center text-gray-400 text-sm py-8">No messages yet. Ask your teacher a question!</p>
                  ) : (
                    conversation.map(m => (
                ...
                <form onSubmit={sendMessage} className="p-4 border-t border-gray-100 dark:border-gray-800 flex gap-3">
                  <input type="text" value={newMsg} onChange={e => setNewMsg(e.target.value)} placeholder="Type a message..." className="input-field flex-1 py-2.5" />
